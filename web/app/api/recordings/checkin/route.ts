...
import { analyzeCheckinRecording, createUploadedRecording } from "@/lib/recordings";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const recording = await createUploadedRecording({
      ...
      file,
      focusSessionId,
      kind: RecordingKind.CHECKIN,
    });

    const analyzed = await analyzeCheckinRecording(recording.id);
